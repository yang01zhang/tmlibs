...
func TestEncodeBytes(t *testing.T) {
	...
	cases := []struct {
		...
		{HexEncoder, []byte{0x1a, 0x2b, 0x3c, 0x4d}, "1A2B3C4D"},
		{B64Encoder, []byte("D!.3s"), "RCEuM3M="},
		{RawB64Encoder, []byte("D!.3s"), "RCEuM3M"},
	}
...
func (d Dings) MarshalJSON() ([]byte, error) {
	return GetByteEncoder().Marshal(d[:])
}

func (d *Dings) UnmarshalJSON(enc []byte) error {
	var ref []byte
	err := GetByteEncoder().Unmarshal(&ref, enc)
	copy(d[:], ref)
	return err
}
...
func TestByteArray(t *testing.T) {
	...
	cases := []struct {
		...
		{HexEncoder, Dings{0x1a, 0x2b, 0x3c, 0x4d, 0x5e}, "1A2B3C4D5E"},
		{B64Encoder, d, "RCEuM3M="},
		{RawB64Encoder, d, "RCEuM3M"},
	}
