...
func SetByteEncoder(e ByteEncoder) {
	if encoder == nil {
		encoder = e
	} else {
		panic("ByteEncoder already set.")
	}
}

func GetByteEncoder() ByteEncoder {
	return encoder
}

// For testing.
func clearByteEncoder() {
	encoder = nil
}
...
//     return data.GetEncoder().Marshal(d[:])
//   }
//
//   func (d *Dings) UnmarshalJSON(enc []byte) error {
//     var ref []byte
//     err := data.GetEncoder().Unmarshal(&ref, enc)
//     copy(d[:], ref)
//     return err
//   }
